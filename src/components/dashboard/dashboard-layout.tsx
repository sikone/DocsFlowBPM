        ...
        <ScrollArea className="flex-1">
          <div className="py-2">
            ...
            >
              ...
              {!sidebarCollapsed && <span className="truncate">Все документы</span>}
              {!sidebarCollapsed && (
                <Badge
                  variant="secondary"
            ...
            {!sidebarCollapsed && rootFolders.length > 0 && (
        ...
        <div className="p-3 space-y-1">
          ...
          {user?.role === 'ADMIN' && (
            <Button
              ...
              className={`
                w-full justify-start gap-2 text-slate-300 hover:text-white hover:bg-slate-800 h-9 text-sm
                ${sidebarCollapsed ? 'px-0 justify-center' : ''}
              `}
            >
              <Settings className="h-4 w-4 shrink-0" />
              {!sidebarCollapsed && 'Настройки'}
            </Button>
          )}
        </div>
      </aside>
...
function DocumentTable({ documents, onDocClick, sortField, sortDir, onSort }: DocumentTableProps) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {documents.map((doc) => (
        <div
          key={doc.id}
          onClick={() => onDocClick(doc.id)}
          className="group bg-white rounded-lg border shadow-sm p-4 cursor-pointer hover:shadow-md hover:border-emerald-200 transition-all"
        >
          <div className="flex items-start gap-3">
            <div
              className="flex items-center justify-center w-10 h-10 rounded-lg shrink-0"
              style={{
                backgroundColor: `${doc.type?.color || '#64748b'}15`,
                color: doc.type?.color || '#64748b',
              }}
            >
              {getDocTypeIcon(doc.type?.systemName || '')}
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-medium truncate group-hover:text-emerald-700 transition-colors">
                {doc.title}
              </h3>
              {doc.number && (
          ...
          {doc.creator && (
            <div className="mt-2 pt-2 border-t flex items-center gap-1.5">
              ...
              <span className="text-xs text-muted-foreground truncate">{doc.creator.name}</span>
            </div>
          )}
        </div>
      ))}
